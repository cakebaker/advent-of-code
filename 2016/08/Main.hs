...
main = do
  ...
  putStrLn $ "Screen for puzzle 2:"
  putStr $ toString $ getFinalScreen initialScreen instructions
...
getLitPixelCount initialScreen instructions = length $ filter id $ elems $ getFinalScreen initialScreen instructions


getFinalScreen :: Screen -> [(Screen -> Screen)] -> Screen
getFinalScreen initialScreen instructions = foldl (\screen f -> f screen) initialScreen instructions
...
toString scr = unlines $ chunkOf screenWidth $ map boolToChar [scr ! (i,j) | i <- [0..(screenHeight - 1)], j <- [0..(screenWidth - 1)]]
               where boolToChar = (\x -> if x then '#' else '.')
